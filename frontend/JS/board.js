...
function crearDadoHTML(valor) {
  ...
  for (let i = 0; i < 9; i++) {
    dots += `<span class="dot${
      puntos[valor].includes(i) ? " filled" : ""
    }"></span>`;
  }
...
fetch("http://127.0.0.1:5000/board")
  .then((res) => res.json())
  .then((data) => {
    // Secciones que tenemos en el JSON
    const sections = ["top", "right", "bottom", "left"];

    sections.forEach((section) => {
      // Selecciona todas las celdas que tienen como texto el nombre de la sección
        const cells = Array.from(document.querySelectorAll(".cell")).filter(
        (cell) => cell.textContent.trim() === section
      );

      // Los items de esa sección en el JSON
      const items = data[section];

      // Rellenamos cada celda con el nombre correspondiente
  ...
  .catch((err) => console.error("Error cargando el tablero:", err));
